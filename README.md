...
1. System packages (`portaudio`, `xdotool`, `xclip`, `libnotify-bin`)
2. Python packages (`faster-whisper`, `sounddevice`, `numpy`, `pynput`)
3. Downloads the Whisper `base.en` model (~74MB, one-time)
...
python3 stt_hotkey.py --hotkey f9

# Key combo
python3 stt_hotkey.py --hotkey super+shift+s

# Use the faster tiny model
python3 stt_hotkey.py --model tiny.en
...
On Wayland, the default `pynput` backend may not capture global hotkeys for native Wayland windows. Use the `evdev` backend instead:
...
For text pasting on Wayland, install:
